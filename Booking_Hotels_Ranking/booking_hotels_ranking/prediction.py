...
def training(df, room_test):
    ...
    x = df.index[df["Room_id"] == f"{room_test}"].astype(int).tolist()
    for i in x:
        t.add_row(
            f"{true[i]:.2f}",
            f"{pred[i]:.2f}",
...
def compare(room_test, true, pred):
    ecart = list()
    for i in range(len(true)):
        ecart.append(((pred[i] - true[i]) / true[i]) * 100)
    ecart = np.around(ecart, decimals=2)
    ...
    x = room_test.index.astype(int).tolist()
    for i in x:
        t.add_row(
            f"{true[i]:.2f}",
            f"{pred[i]:.2f}",
...
# with open("Booking_Hotels_Paris_cleaned_og.json", "r") as read_content:
