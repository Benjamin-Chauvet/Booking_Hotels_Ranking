...
def grade():
    ...
    """  
    df['Hotel_grade'] = df['Hotel_grade'].replace('', 0)
    ...
    for category in list_categories:
        ...
        for i in range(0, len(df)):
            if type(df['Hotel_categories'][i][f'{category}']) == list:
                ...
                df[f'{category}'][i] = df['Hotel_categories'][i][f'{category}']
...
def breakfast_price():
    ...
    for i in range(0, len(df)):
        if '€' in df['Room_breakfast'][i]:
            price = df['Room_breakfast'][i].split('€')[-1]
            if '(optional)' in price:
                df['Room_breakfast_price'][i] = price.split(" ")[1]
            else:
                df['Room_breakfast_price'][i] = price
                df['Room_breakfast_price'][i] = df['Room_breakfast_price'][i][1:]
...
def type_converter():
    ...
    df['Hotel_grade'] = df['Hotel_grade'].replace(0, None)
...
print(df)
df.to_csv('df_1000_Paris.csv', index=False)

json_data = df.to_json()
with open('Booking_Hotels_Paris_cleaned.json', 'w') as file:
